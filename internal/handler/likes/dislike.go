...
func Dislike(env *env.Env) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		userid, err := funcs.GetUserID(db, cookie.Value) // function is in handler/addcomment.go
		...
		*/
		...
		if commentid != "" {
			...
			err = funcs.CheckCommentLikes(db, userid, commentid, 0)
			if err != nil {
				http.Error(w, err.Error(), 500)
				return
			}
			...
			err = funcs.CheckPostLikes(db, postid, userid, 0)
			if err != nil {
				http.Error(w, err.Error(), 500)
				return
			}
